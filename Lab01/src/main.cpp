...
#pragma omp parallel for private(n)
	for (std::size_t i = 0; i < arrowPos.size(); i++) {
		...
		for (std::size_t n = 0; n < particlesPos.size(); n++) {
			...
			fieldVec += glm::vec3(scale * particleCharges[n] / pow(glm::length(dir), 2)) * glm::normalize(dir);
	...
	for (std::size_t i = 0; i < particlesPos.size(); i++){
